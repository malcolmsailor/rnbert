...
python ${HELPER_SCRIPTS_DIR}/predict_keys_with_viterbi.py \
    ...
    output_h5=${RN_PREDS}/viterbi_predictions/${SLURM_ID}_collated/test/key_pc_mode-alpha=${VITERBI_ALPHA}.h5 \
    sticky_viterbi_alpha=${VITERBI_ALPHA}

# Get predictions
python ${HELPER_SCRIPTS_DIR}/get_csv_of_predictions_and_labels.py \
    metadata=${RN_PREDS}/per_salami_slice_logits/${SLURM_ID}_collated/test/${METADATA_BASENAME} \
    predictions_h5=${RN_PREDS}/viterbi_predictions/${SLURM_ID}_collated/test/key_pc_mode-alpha=${VITERBI_ALPHA}.h5 \
    output_path=${RN_PREDS}/viterbi_predictions/${SLURM_ID}_collated/test/key_pc_mode-alpha=${VITERBI_ALPHA}.csv \
...
python ${HELPER_SCRIPTS_DIR}/calculate_metrics_from_csvs.py \
    ${RN_PREDS}/viterbi_predictions/${SLURM_ID}_collated/test/key_pc_mode-alpha=${VITERBI_ALPHA}.csv \
